...
using StateMachine = StaterV.StateMachine.StateMachine;
...
namespace CSharpCodeGenerator
{
    ...
    public class Plugin : ButtonPlugin
    {
        ...
        /// <returns></returns>
        public override PluginRetVal Start(PluginParams _pluginParams)
        {
            ...
            var machineList = _pluginParams.pm.ExportStateMachines();
        ...
        {
            StreamWriter sw = new StreamWriter(_location + "\\Events.cs");
            int indent = 0;
            ...
            var evtList = new Dictionary<string, StaterV.Attributes.Event>();
            foreach (var machine in machineList)
            {
        ...
        private void WriteActionsDeclarations(StreamWriter sr, int _indentCount, StaterV.StateMachine.StateMachine machine)
        {
            var indentCount = _indentCount;
